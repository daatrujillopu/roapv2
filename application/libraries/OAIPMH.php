...
class OAIPMH {
    ...
    function __construct() {
        ...
        $this->CI->load->model("Libraries/oai_model");
        $this->response = "";
...
//echo $this->get_full_query_offset(10);
    ...
    private function get_listRecord() {
        ...
        $objetos = $this->CI->oai_model->execute_query($this->get_full_query_offset($offset));
        //print_r($offset);
        //  $deleted = $this->conexion->realizarOperacion($this->get_listRecordsDeleted_query());
        foreach($objetos as $data){
...
//        }
    ...
    private function get_data_Identify() {
        ...
        $this->add_to_response($this->close("repositoryName", $information[0]['repository_name']));
        $this->add_to_response($this->close("baseURL", SERVER));
        $this->add_to_response($this->close("protocolVersion", "2.0"));
        $this->add_to_response($this->close("adminEmail", $information[0]['email_administrator']));
    ...
    private function get_repository_information() {
        ...
        $result = $this->CI->oai_model->execute_query($query);
...
//agrega la etiqueta del siguiente token
    ...
    private function has_next_resumption_token() {
        ...
        if (($offset + $this->limit_page) < @pg_num_rows($this->CI->oai_model->execute_query($this->get_full_query()))) {
            return true;
        }
    ...
    private function has_listRecords_record() {
        //$va = $this->get_full_query();
        //echo $va;
        $tiene = $this->CI->oai_model->execute_query2($this->get_full_query());
        if($tiene>0){
            return true;
        }else{
            return false;
...
//        return ($result) ? true : false;
    ...
    private function get_xmlo($idlo, $attr = 'xmlo', $text = true) {
        $result =$this->CI->oai_model->execute_query("SELECT xmlo,insertiondate FROM lo WHERE idlo='$idlo'");
        if ($text) {
            return $result;
        } else {
    ...
    private function konsoadd_to_response($text) {
        $this->response.="{$text}";
    }
